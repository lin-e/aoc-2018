...
fun main() {
    ...
    while (carts.size > 1) {
        carts.sortedWith(CartCompare).forEach { c ->
            c.tick()
            if (c.collide()) {
                carts.filter { it.x == c.x && it.y == c.y && !it.remove }.forEach { it.remove() }
            }
        }
        carts = carts.filter { !it.remove }.toMutableList()
    }
    val rem = carts.get(0)
    println("(${rem.x}, ${rem.y})")
...
class Cart {
    ...
    fun collide(): Boolean {
        return carts.filter { it.x == x && it.y == y && !it.remove }.size > 1
    }
}
